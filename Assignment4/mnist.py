...
"""
1. (1 point) Search MNIST dataset at OpenML, it is called "mnist_784".
Download it using sklearn function fetch_openml. Get features and targets.
"""
...
# Note: Reshaping is already done for us above.
mnist_samples, mnist_targets = part_one()

def visualize_a_couple_of_images(samples: np.ndarray, targets: np.ndarray):
    fig = plt.figure()
    title = fig.suptitle("Examples from the MNIST dataset")
    for i in range(4):
        ax: plt.Axes = fig.add_subplot(2,2,i+1)
        ax.imshow(samples[i])
        ax.set_title(f"An example of a {targets[i]}")
        # Turn off tick labels
        ax.set_yticklabels([])
        ax.set_xticklabels([])
    title.set_y(0.95)
    fig.subplots_adjust(top=0.85)
    fig.savefig("mnist_examples.png")
    plt.show()
...
"""
4. (3 point) Filter data leaving only classes 1, 3, 7.
Transform features and targets. How many data points left after filtering?
"""

def filter_examples(features: np.ndarray,
                    labels: np.ndarray,
                    labels_to_keep: List[int]) -> Tuple[np.ndarray, np.ndarray]:
    filter_mask = np.any([labels == value for value in labels_to_keep], axis=0)
    return features[filter_mask], labels[filter_mask]

remaining_x, remaining_y = filter_examples(mnist_samples, mnist_targets, [1, 3, 7])
assert set(remaining_y) == {1, 3, 7}
print(f"There are {remaining_x.shape[0]} data points left \
    after removing everything but classes 1, 3, and 7.")
# Outputs: "There are 22311 data points left after removing everything but classes 1, 3, and 7."
...
Take first 16,000 images and targets as the train, then next 3,000
as validation, then the rest as the test subset.
"""
